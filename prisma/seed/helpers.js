...
const daysFromNow = (n) => new Date(Date.now() + n * MS_PER_DAY);
...
const indianPhone = (i) =>
  `+91${rand(INDIAN_PHONE_PREFIXES)}${String(i + 1).padStart(2, '0')}${randInt(100000, 999999)}`;

const avatarUrl = (name) =>
  `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=random&color=fff&size=128`;

const productImageUrl = (productName) => PRODUCT_IMAGE_MAP[productName] ?? FALLBACK_IMAGE;

const categoryImageUrl = (categoryName) => CATEGORY_IMAGE_MAP[categoryName] ?? FALLBACK_IMAGE;

const blogCoverUrl = (category) => BLOG_COVER_MAP[category] ?? BLOG_COVER_MAP.Health;
