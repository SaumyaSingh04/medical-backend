...
const LEAD_SELECT = {
  ...
  source: true, status: true, cnpCount: true, isDeleted: true, deletedAt: true,
  assignedToId: true, assignedAt: true, orderId: true, formData: true,
...
const NOTE_SELECT = {
  id: true, leadId: true, text: true, direction: true, createdById: true, createdAt: true,
  createdBy: { select: { id: true, firstName: true, lastName: true } },
};

const FOLLOWUP_SELECT = {
  id: true, leadId: true, scheduledAt: true, completedAt: true, status: true,
  note: true, createdById: true, createdAt: true, updatedAt: true,
  createdBy: { select: { id: true, firstName: true, lastName: true } },
};
...
class LeadRepository {
  ...
  async findById(id) {
    return prisma.lead.findFirst({ where: { id, isDeleted: false }, select: LEAD_SELECT });
  }
  ...
  async findAll(filter = {}, { skip = 0, limit = 20 } = {}) {
    return prisma.lead.findMany({ where: buildWhere(filter), select: LEAD_SELECT, orderBy: { createdAt: 'desc' }, skip, take: limit });
  }
  ...
  async softDelete(id) {
    return prisma.lead.update({ where: { id }, data: { isDeleted: true, deletedAt: new Date() }, select: LEAD_SELECT });
  }
  ...
  async findNotes(leadId) {
    return prisma.leadNote.findMany({ where: { leadId }, select: NOTE_SELECT, orderBy: { createdAt: 'desc' } });
  }
  ...
  async findFollowUps(leadId) {
    return prisma.leadFollowUp.findMany({ where: { leadId }, select: FOLLOWUP_SELECT, orderBy: { scheduledAt: 'asc' } });
  }
  ...
  async addTimeline(data) {
    return prisma.leadTimeline.create({ data, select: TIMELINE_SELECT });
  }

  async findTimeline(leadId) {
    return prisma.leadTimeline.findMany({ where: { leadId }, select: TIMELINE_SELECT, orderBy: { createdAt: 'desc' } });
  }

  // ─── Analytics ───────────────────────────────────────────────────────────────

  async countByStatus() {
    return prisma.lead.groupBy({ by: ['status'], where: { isDeleted: false }, _count: { id: true } });
  }

  async countBySource() {
    return prisma.lead.groupBy({ by: ['source'], where: { isDeleted: false }, _count: { id: true } });
  }
